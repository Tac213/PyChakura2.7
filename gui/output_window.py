...
from PyQt4.QtGui import QFrame, QToolButton, QToolBar, QVBoxLayout, QWidgetAction, QCheckBox, QMenu
from PyQt4.QtGui import QIcon, QAction, QCursor, QColor
...
class OutputWindow(QFrame):
    ...
    def _setup_ui(self):
        ...
        self.prev_button.setToolTip(u'上一个匹配项')
        ...
        self.next_button.setToolTip(u'下一个匹配项')
    ...
    def _create_actions(self):
        ...
        Returns:
            None
        """
        self.clear_action = QAction(QIcon(':/res/svg/clear.svg'), u'清除', self)
        self.clear_action.setToolTip(u'清空输出窗口信息')
        self.clear_action.triggered.connect(self._on_clear)

        self.show_menu_action = QAction(QIcon(':/res/svg/menu.svg'),
                                        u'输出类型菜单', self)
        self.show_menu_action.setToolTip(u'显示输出类型菜单')
        self.show_menu_action.triggered.connect(self._on_show_message_menu)

        self.search_action = QAction(QIcon(':/res/svg/search.svg'), u'搜索', self)
        self.search_action.setToolTip(u'在输出窗口中搜索')
    ...
    def _on_console_send(self):
        ...
        Returns:
            None
        """
        script = str(self.console_input.text().toUtf8()).strip()
        ...
        if script:
            for handler in self._console_handlers:
                ...
                self._interpreter.runsource(script.decode('utf-8'))
