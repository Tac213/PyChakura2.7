...
from PyQt4.QtGui import QTextCursor, QBrush, QColor, QTextDocument, QTextBrowser
...
class OutputTextWidget(QTextBrowser):
    ...
    def add_item(self, item, msg_type=MessageType.NORMAL, color=None):
        ...
        if len(item) > self.MAX_LINE_LENGTH:
            ...
            text = pydoc.HTMLDoc.escape(text)
            text += u'<a href="#wrap:%s" style="color:%s">......</a>' % (urllib.quote(left_text).decode('utf-8'),
                                                                         color_name)
        else:
            text = pydoc.HTMLDoc.escape(item)
            text = text.replace('"', '&quot;')
            text = text.replace('\'', '&#x27;')

        left_text = self._process_text(text)
        if color:
            text = u'<span style=\'color:%s;white-space:pre;\'>%s</span>' % (color_name, left_text.decode('utf-8'))
        else:
            text = u'<span style=\'white-space:pre;\'>%s</span>' % left_text.decode('utf-8')
    ...
    def anchor_clicked(self, link):
        ...
        if url.startswith('#wrap:'):
            ...
            ret = '<span style=\'color:%s;white-space:pre;\'>%s</span>'\
                  % (color_name, self._process_text(pydoc.HTMLDoc.escape(block_text)))
            ...
            if not os.path.isfile(filename):
                import py_chakura
                py_chakura.logger.error(u'无法找到文件: %s', filename)
    ...
    def on_find(self):
        ...
        if not self.find_widget:
            return
        self.find_widget.line_edit.setFocus(Qt.OtherFocusReason)
    ...
    def find_text(self, text):
        ...
        if cursor.hasSelection():
            ...
            cursor.movePosition(QTextCursor.Left, QTextCursor.MoveAnchor, select_len)
    ...
    def find_prev(self):
        ...
        if not self.search_text:
            return
        self.find(self.search_text, QTextDocument.FindBackward)
